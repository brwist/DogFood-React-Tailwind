...
import { cancelConstants, otherConstants } from "../../constants";
...
const CancelOptions = ({
  ...
  const pauseMeal = () => {
    ...
    const pauseUntilToSend = pauseType === "1_delivery" || pauseType === "2_deliveries" || pauseType === "forever"
      ? pauseType
  ...
  const options = [
    ...
    subscriptionPhase
      ...
      && subscriptionPhase.status
        !== otherConstants.SUBSCRIPTION_STATUS.WAITING_FOR_TRIAL_SHIPMENT && {
      value: "specific",
      text: (
        <div>
          Pause until
      ...
      <div className="mb-5">
        {isSubscriptionPaused ? (
          <div className="py-6 px-16">
            <div className="flex items-center flex-col mb-4">
              <h2 className="text-xl font-bold mb-5">Your account has been successfully paused</h2>
          ...
          <>
            {pauseBoxType === "MAIN" && (
              <p className="font-cooper text-base font-bold mb-6">
                  ...
                  <div className="bg-yellow-100 p-5 rounded-md mb-3">
                    ...
                    <p className="text-xl font-semibold mb-2">
                      ...
                      <span className="font-medium">
                        {" "}
                        {new Date(subscription.next_billing_at * 1000).toDateString()}
                      </span>
                    </p>
                    )}
                            ...
                            && delivery_starting_date_options.length > 0 && (
                              <p className="text-xl font-semibold mb-2">
                                ...
                                <span className="font-medium">
                                  {" "}
                                  {new Date(
                                    delivery_starting_date_options[0].value * 1000,
                                  ).toDateString()}
                                </span>
                              </p>
                    )}
        ...
        {pauseBoxType === "MAIN" && (
          <div className="flex items-center mt-8">
            <button
              className="rounded-xl py-3 px-8 text-base font-bold bg-primary text-white"
              onClick={() => (pauseType === "cancel" || pauseType === "forever"
                ? setPauseBoxType("REASON")
                : setPauseBoxType("CONFIRM"))}
            >
              {pauseType === "cancel" || pauseType === "forever" ? "Next" : "Confirm"}
            </button>
            <a className="ml-3 mb-0 text-primary font-messina font-bold" href="/">
      ...
      {pauseBoxType === "TIME" && (
        <>
          ...
          <div className="py-6 w-8/12 mx-auto">
            ...
            {pauseUntil && (
              <p className="text-sm mb-4 font-messina">
                Next billing date is
                {" "}
                <b>{moment(pauseUntil).format("LL")}</b>
              </p>
            )}
